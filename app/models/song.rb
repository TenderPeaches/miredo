class Song < ApplicationRecord
    ...
    scope :sort_by_last_played, -> (user_id, order = :desc) { select('songs.*').joins(:song_plays).select('songs.id', 'songs.name', 'songs.capo', 'MAX(song_plays.played_at) AS "last_played_date"').where(song_plays: { user_id: user_id}).group("song_plays.song_id, songs.id, songs.name, songs.capo").order(last_played_date: order) }
    ...
    def hot?(user)
        user = get_user_if_id(user)
        threshold = user.hot_plays_setting
        timelapse = user.hot_days_setting
    ...
    def last_play(user)
        last_play = song_plays.where(user: user).where.not(played_at: nil).order(played_at: :desc).first
    end
